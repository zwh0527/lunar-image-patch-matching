# -*- coding: utf-8 -*-
"""
Created on Fri Apr 15 11:38:05 2022

@author: knight
"""

# 用于MatchNet等基于度量的模型的训练模板
...
from other_model_util import MatchNet, MatchNet_2ch, MatchNet_2ch2stream
from data_util import ImagePatchDataset, get_transforms, get_samples_siam
from train_test_util import get_loss_matchnet, get_acc_matchnet, get_acc_matchnet_2ch

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='train for Matchnet')
    ...
    net = MatchNet_2ch().to("cuda")
    
    if args.pretrain:
      net.load_state_dict(torch.load(os.path.join(root_model, "MatchNet_epoch_" + str(args.pretrain_epoch) + ".pth")))
    ...
    print("This is a model training for Matchnet!")
    ...
    for epoch in range(start_epoch,epochs):
        ...
        for i, (img_map, img_lcam) in enumerate(train_dataloader):
            ...
            count += 1
            loss += E.item()
            acc += get_acc_matchnet_2ch(img_map[0:len(img_map)//2], img_lcam[0:len(img_lcam)//2], net).item()
        ...
        with torch.no_grad():
            ...
            for j, (img_map, img_lcam) in enumerate(test_dataloader):
                ...
                acc_test.append(get_acc_matchnet_2ch(img_map[0:len(img_map)//2], img_lcam[0:len(img_lcam)//2], net).item())
            ...
            if epoch % 2 == 1 and epoch >= 3:
                torch.save(net.state_dict(), os.path.join(root_model, "MatchNet_epoch_" + str(epoch + 1) + ".pth"))
            ...
            if avg_acc_test > best_acc:
                ...
                best_model = copy.deepcopy(net.state_dict())
                torch.save(best_model, os.path.join(root_model, "MatchNet_best_model.pth"))
    f.close()